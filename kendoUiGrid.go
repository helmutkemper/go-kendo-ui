...
type KendoUiGrid struct {
  ...
  // it into Excel or other similar programs that understand TSV/CSV formats.
  // By default allowCopy is disabled and the default format is TSV. Can be set to a JavaScript object which represents
  // the allowCopy configuration. (default: false)
  //
  ...
  //      });
  //    </script>
  AllowCopy interface{} `jsObject:"allowcopy" jsType:"*KendoAllowCopy,Boolean"`

	// @see https://docs.telerik.com/kendo-ui/api/javascript/ui/grid/configuration/altRowTemplate
  //
	// The template [ https://docs.telerik.com/kendo-ui/api/javascript/kendo/methods/template ] which renders the
	// alternating table rows. Be default the grid renders a table row (`<tr>´) for every data source item.
  //
	// Important:
	//
	...
	//    </script>
	//
	AltRowTemplate JavaScript `jsObject:"altRowTemplate"`
  ...
  // ] event of the dataSource instance is fired. By default, `autoBind´ is set to `true´ and the widget will bind to
  // the data source specified in the configuration.
  //
  // Important:
  //
  // Setting `autoBind´ to `false´ is useful when multiple widgets are bound to the same data source. Disabling
  // automatic binding ensures that the shared data source doesn't make more than one request to the remote service.
  // (default: true)
	//
  ...
  //
  // The configuration of the grid columns. An array of JavaScript objects or strings. JavaScript objects are
  // interpreted as column configurations. Strings are interpreted as the field to which the column is bound. The grid
  // will create a column for every item of the array.
  //
  // Important:
  //
  ...
  //
	...
	// kendo.data.DataSource [ https://docs.telerik.com/kendo-ui/api/javascript/data/datasource ] instance using that
  ...
  // rows.
  //
  // Check Detail Template for a live demo [ href="http://demos.telerik.com/kendo-ui/grid/detailtemplate ].
  //
  // Important:
  //
  // The detail template content cannot be wider than the total width of all master columns, unless the detail template
  // is scrollable.
	//
  ...
  //
	// Can be set to a string `phone´ or `tablet´ which will force the widget to use adaptive rendering regardless of
	// browser type.
	//
  ...
  // disabled.(default: false)
	...
	//
	// Sets a value indicating whether the selection will be persisted when sorting, paging, filtering and etc are
	// performed.
  //
	// Important:
	//
	...
	//
	// Selection persistence does not work for new items when the Grid DataSource is in offline mode. In offline mode,
	// newly added items do not have IDs, which are required for selection persistence to work. (default: false)
	//
  ...
  //
	// If set to `true´, users can resize columns by dragging the edges (resize handles) of their header cells. As of
	// Kendo UI Q1 2015, users can also auto-fit a column by double-clicking its resize handle. In this case the column
	// will assume the smallest possible width, which allows the column content to fit without wrapping.
  //
  ...
  //
	...
	//    </script>
	//
	RowTemplate JavaScript `jsObject:"rowTemplate"`
  ...
  //
	//    <ul>
	//      <li>"row" - the user can select a single row.</li>
	//      <li>"cell" - the user can select a single cell.</li>
	//      <li>"multiple, row" - the user can select multiple rows.</li>
	//      <li>"multiple, cell" - the user can select multiple cells.</li>
	//    </ul>
  //
	// Important:
	//
	// When the selectable property is set to "multiple, row" or "multiple, cell" the Grid cannot be scrollable on mobile
	// devices as both are listening on the same event. (default: false)
	//
	...
	//    </script>
	//
	Selectable interface{} `jsObject:"selectable" jsType:"Boolean,string"`
}
